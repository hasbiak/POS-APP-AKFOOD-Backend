  ...
  patchProduct: async (request, response) => {
    ...
    try {
      ...
      if (checkId.length > 0) {
        ...
        if (checkId[0].product_image === 'noimage.png') {
          if (request.file === undefined) {
            setData.product_image = 'noimage.png'
          } else {
          ...
          setData.product_image = checkId[0].product_image
          const result = await patchProduct(setData, id);
          return helper.response(
            response,
            200,
          ...
          setData.product_image = request.file.filename
          const path = `./uploads/${checkId[0].product_image}`;
